...
$(aws ec2 create-tags --resources "$subnet1Id" --tags Key=Name,Value="Subnet1")
echo "----SUBNET 1 CREATED----"


awsSubnet2_response=$(aws ec2 create-subnet --vpc-id "$vpcId" --cidr-block "$subnetCidrBlock2")

subnet2Id=$(echo -e "$awsSubnet2_response" |  /usr/bin/jq '.Subnet.SubnetId' | tr -d '"')
echo $awsSubnet2_response
$(aws ec2 create-tags --resources "$subnet2Id" --tags Key=Name,Value="Subnet2")
echo "----SUBNET 2 CREATED----"


awsSubnet3_response=$(aws ec2 create-subnet --vpc-id "$vpcId" --cidr-block "$subnetCidrBlock3")

subnet3Id=$(echo -e "$awsSubnet3_response" |  /usr/bin/jq '.Subnet.SubnetId' | tr -d '"')
echo $awsSubnet3_response
$(aws ec2 create-tags --resources "$subnet3Id" --tags Key=Name,Value="Subnet3")
...
#add route to subnet
response1=$(aws ec2 associate-route-table --subnet-id "$subnet1Id" --route-table-id "$routeTableId")
response2=$(aws ec2 associate-route-table --subnet-id "$subnet2Id" --route-table-id "$routeTableId")
response3=$(aws ec2 associate-route-table --subnet-id "$subnet3Id" --route-table-id "$routeTableId")
